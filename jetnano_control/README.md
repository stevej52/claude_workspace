...
This is a rewrite of `drivenano.py`, `pijoy.py` and `sync_server.py` from the
`jetnano_python` repository. Same intent and same hardware, new structure:
one small package, a program for each end, tests that run without a robot.
...
```bash
git clone https://github.com/stevej52/claude_workspace
cd claude_workspace/jetnano_control
# operator machine
pip3 install -e ".[pi]"
